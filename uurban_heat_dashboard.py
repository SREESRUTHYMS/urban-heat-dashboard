...
def load_hei_data():
    """
    hei_export confirmed columns:
    gid, geom, hei_score, risk_class, risk_code,
    population, ndvi, lulc_class, urban_class,
    lst_norm, pop_norm, ndvi_inv, lulc_w, color_hex
    """
    q = """
        SELECT
            gid,
            ...
            ROUND(ndvi_inv::numeric, 4)             AS ndvi_inv,
            ROUND(lulc_w::numeric, 4)               AS lulc_w,
            color_hex,
            ST_Y(ST_Transform(geom, 4326))          AS latitude,
            ST_X(ST_Transform(geom, 4326))          AS longitude
        FROM public.hei_export
        WHERE geom IS NOT NULL
        ORDER BY hei_score DESC;
    """
    try:
        conn = get_conn()
        if conn is None:
        ...
        st.warning(f"hei_export load error: {e}")
        return pd.DataFrame()
...
def load_spatial_stats():
    """
    heat_exposure_index confirmed columns:
    geom, pop_raw, ndvi_raw, lulc_class, urban_class,
    lst_norm, pop_norm, ndvi_inv_norm, lulc_weight,
    hei_score, risk_class, risk_code, color_hex
    """
    q = """
        SELECT
            COUNT(*)                                                AS total_pixels,
            ROUND(AVG(hei_score)::numeric, 4)                      AS mean_hei,
            ROUND(MAX(hei_score)::numeric, 4)                      AS max_hei,
            ROUND(MIN(hei_score)::numeric, 4)                      AS min_hei,
            COUNT(CASE WHEN risk_class ILIKE '%high%' THEN 1 END)  AS high_risk_count,
            ROUND(SUM(pop_raw)::numeric, 0)                        AS total_population
    ...
    try:
        ...
        st.warning(f"heat_exposure_index load error: {e}")
        return pd.DataFrame()
...
def load_lst_monthly():
    """
    lst_monthly_stats confirmed columns:
    id, year, month, date_start,
    lst_mean, lst_min, lst_max, lst_stddev
    """
    q = """
        SELECT
            id,
            ...
            month,
            date_start,
            ROUND(lst_mean::numeric, 4)    AS lst_mean,
            ROUND(lst_min::numeric, 4)     AS lst_min,
            ROUND(lst_max::numeric, 4)     AS lst_max,
            ROUND(lst_stddev::numeric, 4)  AS lst_stddev
        FROM public.lst_monthly_stats
        ORDER BY month;
    """
    try:
        conn = get_conn()
        ...
        st.warning(f"LST monthly load error: {e}")
        return pd.DataFrame()


@st.cache_data
def load_hei_risk_summary():
    q = """SELECT * FROM public.hei_risk_summary ORDER BY 1;"""
    try:
        conn = get_conn()
        if conn is None:
            return pd.DataFrame()
...
def load_urban_morphology():
    q = """SELECT * FROM public.urban_morphology LIMIT 500;"""
    try:
        conn = get_conn()
        if conn is None:
            return pd.DataFrame()
...
with tab1:
    ...
    if not stats.empty:
        ...
        kpis = [
            ...
            (c5, f"{int(s['total_population']):,}",    "Population (pop_raw)","#3b82f6"),
    ...
    if not df.empty:
        ...
        with r2:
            ...
            fig_lulc = px.bar(
                lulc_counts, x="Count", y="LULC Class",
                orientation="h",
                color="Count",
...
with tab2:
    ...
    if not df.empty:
        ...
        if show_heatmap:
            heat_data = (df[["latitude", "longitude", "hei_score"]]
    ...
    if not df.empty:
        ...
        with c1:
            fig_hist = px.histogram(
                df, x="hei_score", nbins=40,
                color_discrete_sequence=["#ff6b35"],
                title="HEI Score Histogram"
            )
            fig_hist.update_layout(
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                font_color="#e0e4f0", margin=dict(t=40, b=20)
            )
            st.plotly_chart(fig_hist, use_container_width=True)

        with c2:
            fig_scatter = px.scatter(
                df, x="ndvi", y="hei_score",
                color="risk_class",
                color_discrete_map=risk_colors,
        ...
        # HEI Component Contribution
        st.markdown('<div class="section-header">⚖️ HEI Component Averages by Risk Class</div>',
                    unsafe_allow_html=True)
        components = (
            df.groupby("risk_class")[["lst_norm", "pop_norm", "ndvi_inv", "lulc_w"]]
            .mean().reset_index()
        )
        categories = [
            "LST Norm (w=0.40)",
        ...
        fig_corr = px.bar(
            corr_df, x="Pearson r", y="Variable",
            orientation="h",
            color="Pearson r",
...
with tab4:
    ...
    if not lst_data.empty:
        ...
        st.markdown('<div class="section-header">📊 LST Standard Deviation by Month</div>',
                    unsafe_allow_html=True)
        fig_std = px.bar(
            lst_data, x="month_name", y="lst_stddev",
            color="lst_stddev",
        ...
        st.warning("⚠️ lst_monthly_stats could not be loaded. Check DB connection.")
...
with tab5:
    ...
    if not df.empty:
        table_cols = [
            "gid", "hei_score", "risk_class", "risk_code",
            "population", "ndvi", "lulc_class", "urban_class",
